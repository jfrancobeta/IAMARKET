...
public abstract class NecesidadMapper {
    ...
    public abstract Necesidad toEntity(NecesidadCreateDTO dto, Usuario compania, Estado estado, List<Habilidad> habilidades, Categoria categoria);

    public void updateEntity(Necesidad necesidad, NecesidadUpdateDTO dto, Estado estado, List<Habilidad> habilidades, Categoria categoria){
        if (dto.getTitulo() != null) necesidad.setTitulo(dto.getTitulo());
        if (dto.getDescripcion() != null) necesidad.setDescripcion(dto.getDescripcion());
        if (dto.getCategoria() != null) necesidad.setCategoria(categoria);
        if (dto.getPresupuesto() != null) necesidad.setPresupuesto(dto.getPresupuesto());
        if (dto.getFechaLimite() != null) necesidad.setFechaLimite(dto.getFechaLimite());
        if (dto.getRequirements() != null) necesidad.setRequirements(dto.getRequirements());
        if (dto.getExpectedDeliverables() != null) necesidad.setExpectedDeliverables(dto.getExpectedDeliverables());
        if (estado != null) necesidad.setEstado(estado);
        if (habilidades != null) necesidad.setSkillsRequired(habilidades);
    }
}
